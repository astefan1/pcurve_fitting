...
# Format for simulations

```{r format for simulations}
avg_pcurve_for_sim <- avg_pcurve_for_sim %>%
  select(-c(bin_interval, journal)) %>% 
  mutate(mean_percent = mean_percent/100) %>% 
  pivot_wider(names_from = p_bin_centred, values_from = mean_percent)

#rename for consistency
avg_pcurve_for_sim <- rename(avg_pcurve_for_sim, dataset = year.bin)  
avg_pcurve_for_sim <- rename(avg_pcurve_for_sim, p1 = `0.005`) 
avg_pcurve_for_sim <- rename(avg_pcurve_for_sim, p2 = `0.015`)
avg_pcurve_for_sim <- rename(avg_pcurve_for_sim, p3 = `0.025`) 
avg_pcurve_for_sim <- rename(avg_pcurve_for_sim, p4 = `0.035`)
avg_pcurve_for_sim <- rename(avg_pcurve_for_sim, p5 = `0.045`)

#need to calculate n significant for these time periods at SPPS


 all.sig <- combined_data %>% 
   filter(all.significant == "Yes", journal == "SPPS") 
 
all.sig <- all.sig %>% 
   group_by(year.bin) %>% 
   summarise(nsig = length(unique(doi))) %>% 
   filter(year.bin != "2013-2015" ) %>% 
   rename(dataset = year.bin)
 
 avg_pcurve_for_sim <- left_join(avg_pcurve_for_sim, all.sig, by = "dataset")  
 
 write_csv(avg_pcurve_for_sim, file = "avg_pcurve.csv")
...
```{r}
#calculate avg p curve for each iteration

avg_pcurve_500 <- binned_counts %>%
  filter(journal == "SPPS" & (year.bin == "2010-2012" | year.bin == "2016-2019")) %>% 
  rename(dataset = year.bin) %>% 
  select(-c(bin_interval, journal)) %>% 
  group_by(dataset, p_bin_centred, sample.num) %>%
  summarise(mean_percent = percent/100, .groups = "drop") %>% 
  pivot_wider(names_from = p_bin_centred, values_from = mean_percent) %>% 
  left_join(all.sig, by = "dataset")

#rename for consistency
avg_pcurve_500 <- rename(avg_pcurve_500, p1 = `0.005`) 
avg_pcurve_500 <- rename(avg_pcurve_500, p2 = `0.015`)
avg_pcurve_500 <- rename(avg_pcurve_500, p3 = `0.025`) 
avg_pcurve_500 <- rename(avg_pcurve_500, p4 = `0.035`)
avg_pcurve_500 <- rename(avg_pcurve_500, p5 = `0.045`)


avg_pcurve_for_sim_1 <- avg_pcurve_for_sim

avg_pcurve_for_sim_1$sample.num <- 0 


avg_pcurve_500 <- bind_rows(avg_pcurve_500, avg_pcurve_for_sim_1)

write_csv(avg_pcurve_500, file = "avg_pcurve_500.csv")
